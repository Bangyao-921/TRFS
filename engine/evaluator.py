...
class Evaluator(object):
    ...
    def sliding_eval_rgbX(self, img, modal_x, modal_r, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            if len(modal_r.shape) == 2:
                modal_r_scale = cv2.resize(modal_r, None, fx=s, fy=s, interpolation=cv2.INTER_NEAREST)
            else:
                modal_r_scale = cv2.resize(modal_r, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR)
            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process_rgbX(img_scale, modal_x_scale, modal_r_scale,(ori_rows, ori_cols),
                                                        crop_size, stride_rate, device)
    ...
    def scale_process_rgbX(self, img, modal_x, modal_r, ori_shape, crop_size, stride_rate, device=None):
        new_rows, new_cols, c = img.shape
        long_size = new_cols if new_cols > new_rows else new_rows

        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            input_data, input_modal_x,  input_modal_r, margin = self.process_image_rgbX(img, modal_x, modal_r, crop_size)
            score = self.val_func_process_rgbX(input_data, input_modal_x, input_modal_r, device)
            ...
            modal_r_pad, margin = pad_image_to_shape(modal_r, crop_size, cv2.BORDER_CONSTANT, value=0)
            pad_rows = img_pad.shape[0]
            pad_cols = img_pad.shape[1]
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    if len(modal_x_pad.shape) == 2:
                        ...
                        modal_x_sub = modal_x_pad[s_y:e_y, s_x: e_x,:]
                    if len(modal_r_pad.shape) == 2:
                        modal_r_sub = modal_r_pad[s_y:e_y, s_x: e_x]
                    else:
                        modal_r_sub = modal_r_pad[s_y:e_y, s_x: e_x,:]
                    input_data, input_modal_x, input_modal_r, tmargin = self.process_image_rgbX(img_sub, modal_x_sub, modal_r_sub, crop_size)
                    temp_score = self.val_func_process_rgbX(input_data, input_modal_x, input_modal_r, device)
    ...
    def val_func_process_rgbX(self, input_data, input_modal_x, input_modal_r, device=None):
        ...
        input_modal_r = np.ascontiguousarray(input_modal_r[None, :, :, :], dtype=np.float32)
        input_modal_r = torch.FloatTensor(input_modal_r).cuda(device)
        with torch.cuda.device(input_data.get_device()):
            self.val_func.eval()
            self.val_func.to(input_data.get_device())
            with torch.no_grad():
                score = self.val_func(input_data, input_modal_x, input_modal_r)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
                    input_modal_x = input_modal_x.flip(-1)
                    input_modal_r = input_modal_r.flip(-1)
                    score_flip = self.val_func(input_data, input_modal_x,  input_modal_r)
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                score = torch.exp(score)
    ...
    def process_image_rgbX(self, img, modal_x, modal_r, crop_size=None):
        p_img = img
        p_modal_x = modal_x
        p_modal_r = modal_r
        if img.shape[2] < 3:
        ...
        if len(modal_r.shape) == 2:
            p_modal_r = normalize(p_modal_r, 0, 1)
        else:
            p_modal_r = normalize(p_modal_r, self.norm_mean, self.norm_std)
        if crop_size is not None:
            ...
            p_modal_r, _ = pad_image_to_shape(p_modal_r, crop_size, cv2.BORDER_CONSTANT, value=0)
            p_img = p_img.transpose(2, 0, 1)
            if len(modal_x.shape) == 2:
            ...
            if len(modal_r.shape) == 2:
                p_modal_r = p_modal_r[np.newaxis, ...]
            else:
                p_modal_r = p_modal_r.transpose(2, 0, 1) # 3 H W
            return p_img, p_modal_x, p_modal_r, margin
        ...
        if len(modal_r.shape) == 2:
            ...
            p_modal_r = p_modal_r.transpose(2, 0, 1)  # 3 H W
        return p_img, p_modal_x, p_modal_r
