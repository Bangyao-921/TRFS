...
def random_mirror(rgb, gt, modal_x,modal_r):
    if random.random() >= 0.5:
        rgb = cv2.flip(rgb, 1)
        gt = cv2.flip(gt, 1)
        modal_x = cv2.flip(modal_x, 1)
        modal_r = cv2.flip(modal_r, 1)
    return rgb, gt, modal_x,modal_r

def random_scale(rgb, gt, modal_x,modal_r, scales):
    ...
    modal_r = cv2.resize(modal_r, (sw, sh), interpolation=cv2.INTER_LINEAR)
    return rgb, gt, modal_x,modal_r, scale
...
class TrainPre(object):
    ...
    def __call__(self, rgb, gt, modal_x,modal_r):
        rgb, gt, modal_x,modal_r = random_mirror(rgb, gt, modal_x,modal_r)
        if config.train_scale_array is not None:
            rgb, gt, modal_x,modal_r, scale = random_scale(rgb, gt, modal_x,modal_r, config.train_scale_array)

        rgb = normalize(rgb, self.norm_mean, self.norm_std)
        modal_x = normalize(modal_x, self.norm_mean, self.norm_std)
        modal_r = normalize(modal_r, self.norm_mean, self.norm_std)
        ...
        p_modal_r, _ = random_crop_pad_to_shape(modal_r, crop_pos, crop_size, 0)
        p_rgb = p_rgb.transpose(2, 0, 1)
        p_modal_x = p_modal_x.transpose(2, 0, 1)
        p_modal_r = p_modal_r.transpose(2, 0, 1)
        return p_rgb, p_gt, p_modal_x,p_modal_r
