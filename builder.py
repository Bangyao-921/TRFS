...
class EncoderDecoder(nn.Module):
    ...
    def encode_decode(self, rgb, modal_x, modal_r):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        orisize = rgb.shape
        x = self.backbone(rgb, modal_x, modal_r)
    ...
    def forward(self, rgb, modal_x, modal_r ,label=None):
        if self.aux_head:
            out, aux_fm = self.encode_decode(rgb, modal_x, modal_r)
        else:
            out = self.encode_decode(rgb, modal_x, modal_r)
