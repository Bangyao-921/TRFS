...
class RGBXDataset(data.Dataset):
    def __init__(self, setting, split_name, preprocess=None, file_length=None):
        ...
        self._r_path = setting['r_root']
        self._r_format = setting['r_format']
        self._r_single_channel = setting['r_single_channel']
    ...
    def __getitem__(self, index):
        ...
        r_path = os.path.join(self._r_path, item_name + self._r_format)
        ...
        if self._r_single_channel:
            r = self._open_image(r_path, cv2.IMREAD_GRAYSCALE)
            r = cv2.merge([r, r, r])
        else:
            r =  self._open_image(r_path, cv2.COLOR_BGR2RGB)
        if self.preprocess is not None:
            rgb, gt, x, r = self.preprocess(rgb, gt, x, r)
        ...
        output_dict = dict(data=rgb, label=gt, modal_x=x,models_r=r, fn=str(item_name), n=len(self._file_names))
